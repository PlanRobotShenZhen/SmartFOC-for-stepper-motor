	...
	switch (cs) {
		...
		case 3:
			...
			if (whoami == SDO_SERVER) {
                ...
                //LOGE("--%d", nbBytes);
				if (nbBytes == 0) {
					MSG_ERR(0x3AA6, "SDO End download expedited. Response received. from nodeId", nodeId);
					StopSDO_TIMER(line)
						d->transfers[line].state = SDO_FINISHED;
					if(d->transfers[line].Callback) (*d->transfers[line].Callback)(d,nodeId);
					return 0x00;
		...
		UNS32 * abortCode)
{
	...
	* abortCode = 0;
	...
	if (err) {
		...
		//SYS_DEBUG(("SDO error : No line found for communication with node : ", nodeId));
        *size = 0;
        return SDO_ABORTED_INTERNAL;
	}
